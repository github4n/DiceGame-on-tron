<template>
  <div class='table-list'>
            ...
            <table v-if="newDataList[0]!=undefined" border="0" cellspacing="0"  cellpadding="0">
              <tbody>
               <tr v-for="(item, index) in newDataList" :key='index'>
                  <td v-if="item.bonus == 0" style="color:red;cursor: pointer;" @click="showFair(item.betId)">{{$t('inviterPage.Verification')}}</td>
                  <td v-else style="color:#00FAAC;cursor: pointer;" @click="showFair(item.betId)">{{$t('inviterPage.Verification')}}</td>

                  <td v-if="item.bonus == 0" style="color:red;">{{item.addr}}</td>
                  <td v-else style="color:#00FAAC;">{{item.addr}}</td>
    ...
    <dice-fair :baseInfo='baseInfo' v-show="showFairDialog" @closeFairModule='closeFairModule'></dice-fair>
  </div>
</template>

<script>
import diceFair from '../../../../components/dialog/fair-dialog.vue';
   ...
   /*  tableRowClassName({row, rowIndex}) {
    ...
    getBetRecordList(){
      ...
      getBetRecordList(params).then(result => {
        ...
        this.$store.commit("SET_LETNUM",result[0].lotteryNumber);
      })
    },
