<template>
  <div class='app-banner-box'>
    <a name="myTickets"></a>
    <div>
      <div>
        <div class='app-my-reward'>
          <span style='font-size:12px;width:40%;'>{{$t('betting.MyTicket')}} ( {{TicketList.length}} )</span>
          <el-input style='width:50%;' class="input"  placeholder="" v-model="search" clearable></el-input>
          <!-- &nbsp;<i style='cursor:pointer;font-size: 22px;' class="el-icon-search"> </i>-->&nbsp;&nbsp;
          <i style='cursor:pointer;width:10%;' v-show="down" @mousedown="tabChange('1')" class="el-icon-arrow-down"></i>
          <i style='cursor:pointer;width:10%;' v-show="up" @mousedown="tabChange('2')" class="el-icon-arrow-up"></i>
        </div>
      </div>
      <div v-show="showTab">
