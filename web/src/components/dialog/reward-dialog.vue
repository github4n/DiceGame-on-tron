    ...
    <div class="player-box">
      ...
      <div v-show='selIndex == 1' class='event-reward'>
        <div class='reward-inp'>
          ...
          <el-tooltip class="reward-item" effect="dark" placement="top">
            <div slot="content" class="reward-title" style="width:300px;">
              <div>
      ...
      <div v-show='selIndex == 2'  class='event-detail'>
          ...
          <!-- <div class='detail-list' v-for='(item,index) in detailList' :key='index'>
            <div class='detail-time'>{{item.timestamp | dateFormat}}</div>
            <div class='detail-matter'>{{item.desc}}</div>
            <div class='detail-num'>{{item.amt}}</div>
          </div> -->
          <div class="border-tab">
            <el-table class="pc-invite-tab" :row-style="getRowClass"  :show-header ='false' :data="detailList.slice((currentSize-1)*pageSize,currentSize*pageSize)" >
              <el-table-column width='200' align="center" label="Date" prop="timestamp" :formatter="dateFormat"></el-table-column>
              <el-table-column width='150' align="center" label="Name" prop="desc"></el-table-column>
              <el-table-column width='150' align="center" label="Name" prop="amt"></el-table-column>
            </el-table>
          </div>
  ...
  export default {
    ...
    mounted(){
      ...
      if(this.langularType=='en'){
        $(".pc-invite-tab .el-table__empty-block .el-table__empty-text").text("No Data");
      }
    },
    ...
    methods: {
      ...
      getOdinPrize(){
        ...
        //let params = {Addr: 'TCiGREKREpbLQjbm9YgWbYLxDdsW1tL5dV'}
...
<style lang="less">
  ...
  .pc-invite-tab.el-table td, .el-table th.is-leaf{
    border-bottom:1px solid #00E9E1;
  }
  .pc-invite-tab.el-table,.el-table__expanded-cell {
      background-color: rgba(190, 195, 197, 0);
  }
  .pc-invite-tab.el-table--enable-row-hover .el-table__body tr:hover>td{
    background-color: rgba(190, 195, 197, 0) !important;
  } 
  .pc-invite-tab .el-table__empty-block{
    background-color: rgba(190, 195, 197, 0);
    border-bottom:1px solid #00E9E1;
  }
  .pc-invite-tab .el-table__empty-text{
    color:#00E9E1;
  }
